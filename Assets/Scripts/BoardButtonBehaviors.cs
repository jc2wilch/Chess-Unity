...
public class BoardButtonBehaviors : MonoBehaviour
{
    ...
    Dictionary<string, Func<string, GameObject, bool, List<GameObject>>> moveFunctions = new Dictionary<string, Func<string, GameObject, bool, List<GameObject>>>();
    ...
    void Start()
    {
        ...
        foreach (GameObject item in whitePieces)
        {
            ...
            List<GameObject> possMoves = moveFunctions[item.transform.GetChild(2).GetChild(0).name]("White", item.transform.parent.gameObject, false);//GetMoves("White", item.transform.parent.gameObject, item);
        ...
        foreach (GameObject item in blackPieces)
        {
            ...
            List<GameObject> possMoves = moveFunctions[item.transform.GetChild(2).GetChild(0).name]("Black", item.transform.parent.gameObject, false);
            possMoves.RemoveAll(x => x == null);
    ...
    private List<GameObject> GetPawnMoves(string color, GameObject tile, bool ignore)
    {
    ...
    private List<GameObject> GetRookMoves(string color, GameObject tile, bool ignore)
    {
    ...
    private List<GameObject> GetBishopMoves(string color, GameObject tile, bool ignore)
    {
        char tileChar = tile.name[0];
        char tileNum = tile.name[1];
        char newTileChar = tileChar;
    ...
    private List<GameObject> GetKnightMoves(string color, GameObject tile, bool ignore)
    {
        char tileChar = tile.name[0];
        char tileNum = tile.name[1];
        char newTileChar = tileChar;
    ...
    private List<GameObject> GetKingMoves(string color, GameObject tile, bool ignore)
    {
        char tileChar = tile.name[0];
        char tileNum = tile.name[1];
        char newTileChar = tileChar;
    ...
    private List<GameObject> GetQueenMoves(string color, GameObject tile, bool ignore)
    {
        List<GameObject> possibleMoves = new List<GameObject>();
        possibleMoves.AddRange(GetRookMoves(color, tile, ignore));
        possibleMoves.AddRange(GetBishopMoves(color, tile, ignore)); 
    ...
    private List<GameObject> GetMoves(string color, GameObject tile, GameObject playerPiece)
    {
        ...
        try
        {
            possibleMoves = moveFunctions[BoardDict[color][playerPiece]["Type"]](color, tile, false);
        }
        catch (KeyNotFoundException e)
        {
        ...
        possProtMoves = moveFunctions["Queen"](color, tile, true);
        ...
        foreach (GameObject possMove in possibleMoves)
        {
            ...
            if (protecting)
            {
                ...
                {
                    print('a');
                    foreach (GameObject oppPiece in possibleMoveDict[oppColor][tile.name].Keys.Distinct())
                    {
                        ...
                        {
                            possibleOppMoves.AddRange(moveFunctions[BoardDict[oppColor][oppPiece]["Type"]](oppColor, oppPiece.transform.parent.gameObject, true));
                        }
                    }
        ...
        {
            playerPiece.transform.SetParent(move.transform);

            if (possibleMoveDict[oppColor].ContainsKey(move.name))
            {
                foreach (GameObject key in possibleMoveDict[oppColor][move.name].Keys)
                {
                    possibleOppMoves.AddRange(possibleMoveDict[oppColor][move.name][key]);
                }
            }
            else
            {
            ...
            foreach (GameObject oppMove in possibleOppMoves)
            {
                if (tilesDict[oppMove.name]["Piece"] != null)
                {
                    // check if player king is on move
                    if (tilesDict[oppMove.name]["Piece"].transform.GetChild(1).name == color && tilesDict[oppMove.name]["Piece"].transform.GetChild(2).GetChild(0).name == "King")
                    {
                        invalid = true;
                        break;
        ...
        tilesDict[origParent.name]["Piece"] = playerPiece;
        ...
        // absolute nightmare of nested foreach loops and if statements :(


        foreach (GameObject move in possibleMoves) // checking all of the player's possible moves
        {
            playerPiece.transform.SetParent(move.transform);

            BoardDict[color][playerPiece]["Tile"] = move.name;
            try
            {
                foreach (GameObject oppPiece in BoardDict[oppColor].Keys) // getting all of the opponents pieces to check their possible moves to check if the player's king is on one of them
                {
                    
                    try
                    {
                        possibleOppMoves = moveFunctions[BoardDict[oppColor][oppPiece]["Type"]](oppColor, oppPiece.transform.parent.gameObject, true);
                    }
                    catch (KeyNotFoundException e)
                    {
                        print(BoardDict[oppColor][oppPiece]["Type"]);
                        print(oppPiece.name);
                        print(oppPiece.transform.parent.name);
                        print(3);
                        print(e);
                    ...
                    foreach (GameObject possTile in possibleOppMoves) // tile that the piece can move to
                    {
                        if (possTile == null)
                        {
                            print(777);
                        ...
                        try
                        {
                            if (possTile.transform.childCount > 0)
                            {
                                foreach (Transform child in possTile.transform) // go thru objects inside tile
                                {
                                    if (child.childCount > 2) // if the object has at least 3 children (high likelihood of being a piece)
                                    {
                                        if (child.GetChild(0).name == "Piece") // if object is a piece
                                        {
                                            if (child.GetChild(1).name == color && child.GetChild(2).GetChild(0).name == "King") // if it's the player's king
                                            {
                                                if (move.name == oppPiece.transform.parent.name)
                                                {
                                                    //print("Valid Take");
                                                    break;
                                                }
                                                ...
                                                //print("possTile.name = " + oppPiece.transform.parent.name);
                                                //print("Invalid Move: " + move.name);
                                                invalid = true;
                                                break;
                                            }
                                            //print(child.name);
                    ...
                    if (piece != null)
                    {
                        ...
                        if (child == dot.transform)
                        {
                            ...
                            foreach (GameObject key in BoardDict["White"].Keys) {
                                List<GameObject> possibleMoves = moveFunctions[key.transform.GetChild(2).GetChild(0).name]("White", key.transform.parent.gameObject, false);//GetMoves("White", item.transform.parent.gameObject, item);
                                possibleMoves.RemoveAll(x => x == null);
                            ...
                            {
                                List<GameObject> possibleMoves = moveFunctions[key.transform.GetChild(2).GetChild(0).name]("Black", key.transform.parent.gameObject, false);//GetMoves("White", item.transform.parent.gameObject, item);
                                possibleMoves.RemoveAll(x => x == null);
                            ...
                            possMoves.AddRange(moveFunctions[piece.transform.GetChild(2).GetChild(0).name](piece.transform.GetChild(0).name, tile, false));
                            possMoves.RemoveAll(x => x == null);
